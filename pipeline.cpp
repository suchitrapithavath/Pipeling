...
void Pipeline::cycle(int param) {
    ...
    { 
        ...
        if(forwarding && param==1)
        { 
            
            
            if(pipeline[MEM].inst->dest ==  pipeline[DECODE].inst->src1)
            {
            	pipeline[DECODE].inst->src1 == pipeline[MEM].inst->dest;
            }
            else if(pipeline[MEM].inst->dest ==  pipeline[DECODE].inst->src2)
            {
            	pipeline[DECODE].inst->src2 == pipeline[MEM].inst->dest;
            }
                

               
                    
                else {pipeline[EXEC].addInstruction(new Instruction());
                     return;}
            }

   
    else if(forwarding && param==2)
        {
            if(pipeline[MEM].inst->dest ==  pipeline[DECODE].inst->src1)
            {
            	pipeline[DECODE].inst->src1 == pipeline[MEM].inst->dest;
            ...
            else if(pipeline[WB].inst->dest ==  pipeline[DECODE].inst->src1)
            {
            	pipeline[DECODE].inst->src1 == pipeline[WB].inst->dest;
            }
            else if(pipeline[WB].inst->dest ==  pipeline[DECODE].inst->src2)
            {
            	pipeline[DECODE].inst->src2 == pipeline[WB].inst->dest;
            }
            else
            {
		...
		else{// If dependency detected, stall by inserting NOP instruction
		pipeline[EXEC].addInstruction(new Instruction());
		return;
	}
    }
